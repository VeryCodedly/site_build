...
class Command(BaseCommand):
    help = 'Safely upload existing images to Cloudinary and update DB'

    def add_arguments(self, parser):
        parser.add_argument(
            '--dry-run',
            action='store_true',
            help='Print what would be uploaded without updating DB',
        )
    ...
    def handle(self, *args, **options):
        ...
        for model, field_name in models:
            ...
            for obj in qs:
                ...
                upload_source = getattr(image_field, 'path', None)
                if not upload_source:
                    self.stdout.write(self.style.WARNING(
                        f"No local file found for {obj}, skipping"
                    ))
                    continue

                folder = os.path.dirname(image_field.name) or None

                self.stdout.write(f"{'Dry run: would upload' if dry_run else 'Uploading'} {upload_source} for {obj} to folder '{folder}'...")
        ...
        self.stdout.write(self.style.SUCCESS(f"All done! Updated {total_updated} images."))
